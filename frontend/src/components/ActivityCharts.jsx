...
export function ActivityCharts({report, demo, createdAt}) {
 ...
 const chartTip=(i,x,y)=>{setIndex(i);show([0,1].map(j=>`${report.year-j} · ${(j?previous:current)[i]?.date || '—'} · ${lines[j][i] ?? '—'} روز فعال`).join('\n'),x,y)};
 return <>
 <section className="surface persistence-chart">
 <h3>پشتکار؛ تداوم مشارکت امسال و پارسال</h3>
 <div className="chart-legend"><span><i/>امسال · {report.year}</span><span><i/>پارسال · {report.year-1}</span></div>
 <svg viewBox="0 0 760 250" role="img" aria-label="نمودار مقایسهٔ روزهای فعال تجمعی" dir="ltr" tabIndex={0}
 ...
 <h3>ریتم مشارکت {selectedYear}</h3>
