...
export function Reports({ demo, onImage, createdAt, selection, setSelection }) {
    ...
    queryFn: () =>
      demo
        ? {
            ...
            previous: {
              days: Array.from({ length: 180 }, (_, i) => ({
                date: new Date(Date.UTC(comparisonYear || year, 0, i + 1)).toISOString().slice(0, 10),
                count: i % 4 === 0 ? 0 : (i * 7 + year - 1 - (comparisonYear || year)) % 9,
              })),
        ...
        : api(
            comparisonYear && comparisonYear !== year - 1
              ? `/api/me/activity?compare=${comparisonYear}`
              : "/api/me/activity",
          ),
  });
  const date = (value) =>
  ...
  const fact = (period, key) => {
    ...
    if (key === "bestDay") {
      const peak = stats.bestDates?.at(-1);
      return peak ? `${date(peak)} · ${number(stats.bestDay)}` : null;
    }
    if (key === "longest") {
      const range = stats.longestRange;
      if (!range || !stats.longest) return null;
      const span =
        range[0] === range[1] ? date(range[0]) : `${date(range[0])} — ${date(range[1])}`;
      return `${span} · ${number(stats.longest)} ${t("روز")}`;
    }
    return null;
  };
  const tooltip = (key, hint) => {
    const lines = [hint];
    const current = fact(report.data.current, key);
    if (current) lines.push(`${t("امسال")}: ${current}`);
    if (comparisonYear) {
      const previous = fact(report.data.previous, key);
      if (previous) lines.push(`${comparisonYear}: ${previous}`);
    }
    return lines.join("\n");
  };
    ...
    <>
      <div className="section-heading">
        <div className="report-heading">
          <h2>{t("مسیر مشارکت‌های تو")}</h2>
          {years.length > 1 && (
            <select
              className="report-year-select"
              aria-label={t("سال مقایسه")}
      ...
      {report.isFetching && (
        <LoadingState
          variant={report.data ? "inline" : "report"}
          label={t("در حال خواندن فعالیت‌های GitHub…")}
        />
      )}
      {report.isError && (
                ...
                detail={
                  comparisonYear
                    ? comparisonYear +
                      ...
                      " · " +
                      (p
                        ? number(Math.round(((c - p) / p) * 1000) / 10) +
                          t("٪ نسبت به سال انتخاب‌شده")
                        : c
                          ? t("در بازهٔ مقایسه موردی ثبت نشده")
                          : t("بدون تغییر"))
                    : ""
                }
                extra={tooltip(key, hint)}
              />
            );
          })}
      </div>
      {report.data && (
        <ActivityCharts
          report={{
            ...report.data,
            comparisonYear,
            previous: comparisonYear ? report.data.previous : { days: [] },
          }}
          demo={demo}
          createdAt={createdAt}
        />
